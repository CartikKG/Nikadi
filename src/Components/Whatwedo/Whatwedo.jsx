import React from 'react'

function Whatwedo() {
  return (
    <div>
      <section id="success-story">
        <h1 class="sec-heading">What NIKADI GROUP does</h1>

        <div class="slider">
          <div class="col-6 slide-text">
            <div>
              <h2>World Travel Protection</h2>
              <p>
                Lorem Ipsum is simply dummy text of the printing and typesetting
                industry. Lorem Ipsum has been the industry's standard dummy
                text. Lorem ipsum dolor sit amet, consectetur adipisicing elit.
                Nihil iste eum ipsam dolorum pariatur possimus eligendi eos
                assumenda asperiores commodi voluptates atque repudiandae animi,
                obcaecati facilis blanditiis illo, consectetur sapiente. Optio,
                quidem.
              </p>
              <a href="" class="brand-btn">
                Contact Us
              </a>
            </div>
          </div>
          <div class="col-6 slide-img">
            <img
              src="https://www.dropbox.com/s/ipx91osglyczpdt/delivery_experience.svg?raw=1"
              alt="World Travel App Development"
              title="World Travel Protection"
            />
          </div>
        </div>
      </section>
    </div>
  )
}

export default Whatwedo
